...
## Mis semillas
semilla_primigenia <- 310019

## Working Directory
path <- "G:/Mi unidad/Maestría en Ciencia de Datos - Universidad Austral/Materias/7. Laboratorio de implementación (martes)/Experimentos"
path_dataset_pequeno <- paste0(path,"/datasets/dataset_pequeno.csv")
...
PARAM$rpart$cp <- -1
PARAM$rpart$minsplit <- 1484
PARAM$rpart$minbucket <- 271
PARAM$rpart$maxdepth <- 15
...
setwd(path)
experimento <- "exp/exp4020"
dir.create(experimento)
path_exp <- paste0(path,"/",experimento)
...
for (arbolito in seq(PARAM$num_trees_max) ) {
  ...
  if (arbolito %in% grabar) {
    ...
    arch <- paste("-f", shQuote(file.path(path_exp, archivo_kaggle)))
    mensaje <- paste0("-m ", shQuote(
      paste0("cp=", PARAM$rpart$cp,
